...
/*
* 
* Copyrigh Invilab 2022: Edits by Seppe Sels
* 
...
void find_corners_resized(const cv::Mat& img, Corner& corners, const Params& params) {
  ...
  // refinement
  std::vector<int> choose(corners_resized.p.size(), 0);
  refine_corners(img_du, img_dv, img_angle, img_weight, corners_resized, params,choose);
  if(params.show_processing) {
...
void find_corners(const cv::Mat& img, Corner& corners, const Params& params) {
  ...
  if(img.channels() == 3) {
    ...
    img_norm.convertTo(img_norm, CV_64F, 1. / 255., 0);
      ...
      if (img.type() == CV_64F)
      {
          img.convertTo(img_norm, CV_64F, 1., 0);
      }
      else
      {
  ...
  refine_corners(img_du, img_dv, img_angle, img_weight, corners, params,choose);
  if(params.show_processing) {
    printf("Refining corners (%d x %d) ... %lu\n", img_norm.cols, img_norm.rows, corners.p.size());
  }
  if(params.show_debug_image) {
...
void find_subpixel_corners(const cv::Mat& img, Corner& corners, const Params& params) {
    // clear old data
