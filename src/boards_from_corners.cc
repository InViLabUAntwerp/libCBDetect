...
namespace cbdetect {
    ...
    std::vector<Board> boards_from_corners(const cv::Mat& img, const Corner& corners, const Params& params) {
        ...
        while (n++ < corners.p.size()) {
            ...
            // grow boards
            cv::Point3i maxE_posold = board_energy(corners, board, params);
            double energyold = board.energy[maxE_pos.y][maxE_pos.x][maxE_pos.z];
            ...
            while (1) {
                ...
                for (int j = 0; j < (params.corner_type == MonkeySaddlePoint ? 6 : 4); ++j) {
                    ...
                    std::vector<int> usednew = used;
                    GrowType grow_type = grow_board(corners, usednew, boardnew, proposal, j, params);
                    cv::Point3i maxE_posnew = board_energy(corners, boardnew, params);
                    double energynew = boardnew.energy[maxE_posnew.y][maxE_posnew.x][maxE_posnew.z];
